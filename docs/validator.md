...
|                    [lowercase](https://pkg.go.dev/github.com/go-playground/validator/v10#hdr-Lowercase_String)                    |[uppercase](https://pkg.go.dev/github.com/go-playground/validator/v10#hdr-Uppercase_String)|[datetime](https://pkg.go.dev/github.com/go-playground/validator/v10#hdr-Datetime)|[timezone](https://pkg.go.dev/github.com/go-playground/validator/v10#hdr-TimeZone)|[omitempty](https://pkg.go.dev/github.com/go-playground/validator/v10#hdr-Omit_Empty) |

### 扩展的验证函数

|  -     |    -     |    -     | - |
|:------------:|:-----------------:|:-------------------:|:---------------------:|
| [regex](https://github.com/ntt360/gin/blob/master/docs/validator.md#regex) | [mobile](https://github.com/ntt360/gin/blob/master/docs/validator.md#mobile) | [idcard](https://github.com/ntt360/gin/blob/master/docs/validator.md#idcard) |[omitabsent](https://github.com/ntt360/gin/blob/master/docs/validator.md#omitabsent)|
...
官方提供的 `omitempty` 当字段为空，或者为 `False`、0、null 等0值，验证器会被忽略。但有时候我们希望前端请求的时候如果字段存在则验证，有些场景下传零值是有意义的。那么这时候可以使用 `omitabsent` 验证器。

```go
type Params struct {
    Page int `form:"page" binding:"omitabsent,numeric,min=1" msg:"页码不正确"`
}
```

`omitabsent` 表示：如果前端请求是，存在 `page` 字段，那么验证 `numeric` 和 `min=1`，否则忽略验证（为空也会被验证！）。
